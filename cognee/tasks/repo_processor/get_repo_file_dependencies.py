...
from cognee.tasks.repo_processor.get_local_dependencies import (
    ...
    get_local_script_dependencies_lua,
)


async def get_source_code_files(repo_path, extensions):
    """Get files with specified extensions and their source code"""
...
async def get_repo_file_dependencies(
    repo_path: str, detailed_extraction: bool = False, language: str = "python"
) -> AsyncGenerator[DataPoint, None]:
    """Generate a dependency graph for files in the given repository path based on the specified language."""
    ...
    if language == "python":
        extensions = [".py"]
        get_local_script_dependencies_func = get_local_script_dependencies
    elif language == "lua":
        extensions = [".lua"]
        get_local_script_dependencies_func = get_local_script_dependencies_lua
    else:
        raise ValueError(f"Unsupported language: {language}")

    source_code_files = await get_source_code_files(repo_path, extensions)
    ...
    for start_range, end_range in chunk_ranges:
        ...
        tasks = [
            get_local_script_dependencies_func(repo_path, file_path, detailed_extraction)
